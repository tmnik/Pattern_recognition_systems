...
%% Лабораторная работа №4 по системам распознавания образов. Часть 2
...
imshow(B);
title({'Изображение кадра при повороте кадра';name2});
...
imshow(R,[]);
title({'Корреляционная функция. Преобразование Фурье при повороте кадра'});
...
imshow(A);
title({'Максимум корреляционной функции. Найденное положение при повороте кадра'});
